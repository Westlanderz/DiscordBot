#include "../../include/commands/removeadmin.hpp"
...
RemoveAdmin::RemoveAdmin(std::vector<std::string> names): Command(names) {}

std::string RemoveAdmin::getHelpMessage() {
    return "Use " + module->isHandler()->isPrefix() + names[0] + " <role> to remove that role to the list of Admin roles.";
}

void RemoveAdmin::execute(dpp::Message msg) {
    try {
            ...
            for(auto &role : role_name) {
                ...
                for(std::size_t i = 0; i < response["body"].size(); ++i) {
                    if(role == response["body"].at(i)["id"] && std::count(roles.begin(), roles.end(), role)) {
                        ...
                        std::cout << "\033[1;36m" << role << "\033[1;35m removed from the adminlist for guild " << guild.at(0)["id"] << ".\033[0m" << std::endl;
                        bot->sendMessage(*msg.channel_id, false, "Removed the role for you.");
                ...
                load_msg = "Unable to find this role or this role is not yet added.";
...
bool RemoveAdmin::hasPermsToRun(dpp::User user) {
